...
void readVerilog(fstream& file, vector<string> &inputs, vector<string> &outputs, vector<string> &otherWires, map<string,gates*> &GATES, map<string,wire> &wires){
    ...
    while(!file.eof()){
        ...
        handleDelayedGates(line,GATES,wires);
    }
}
...
void handleDelayedGates(string line, map<string,gates*>& GATES, map<string,wire>& wires){
    if(!isNot(line)){
        regex ex("^[ ]*nand[ ]*[#]([[:w:]]+)[ ]+([[:w:]]+)[ ]*[\(][ ]*([[:w:]]+)[ ]*[,][ ]*([[:w:]]+)[ ]*,[ ]*([[:w:]]+)[ ]*[\)][ ]*[;].*");
        smatch match;
        if(regex_search(line, match, ex)){
            GATES.insert(pair<string,gates*>(match[2],new NAND(wires.find(match[4])->second, wires.find(match[5])->second, wires.find(match[3])->second, stoi(match[1]))));
            return;
        }

        ex="^[ ]*and[ ]*[#]([[:w:]]+)[ ]+([[:w:]]+)[ ]*[\(][ ]*([[:w:]]+)[ ]*[,][ ]*([[:w:]]+)[ ]*,[ ]*([[:w:]]+)[ ]*[\)][ ]*[;].*";
        if(regex_search(line, match, ex)){
            GATES.insert(pair<string,gates*>(match[2],new AND(wires.find(match[4])->second, wires.find(match[5])->second, wires.find(match[3])->second, stoi(match[1]))));
            return;
        }

        ex="^[ ]*or[ ]*[#]([[:w:]]+)[ ]+([[:w:]]+)[ ]*[\(][ ]*([[:w:]]+)[ ]*[,][ ]*([[:w:]]+)[ ]*,[ ]*([[:w:]]+)[ ]*[\)][ ]*[;].*";
        if(regex_search(line, match, ex)){
            GATES.insert(pair<string,gates*>(match[2],new OR(wires.find(match[4])->second, wires.find(match[5])->second, wires.find(match[3])->second, stoi(match[1]))));
            return;
        }

        ex="^[ ]*nor[ ]*[#]([[:w:]]+)[ ]+([[:w:]]+)[ ]*[\(][ ]*([[:w:]]+)[ ]*[,][ ]*([[:w:]]+)[ ]*,[ ]*([[:w:]]+)[ ]*[\)][ ]*[;].*";
        if(regex_search(line, match, ex)){
            GATES.insert(pair<string,gates*>(match[2],new NOR(wires.find(match[4])->second, wires.find(match[5])->second, wires.find(match[3])->second, stoi(match[1]))));
            return;
        }

        ex="^[ ]*xor[ ]*[#]([[:w:]]+)[ ]+([[:w:]]+)[ ]*[\(][ ]*([[:w:]]+)[ ]*[,][ ]*([[:w:]]+)[ ]*,[ ]*([[:w:]]+)[ ]*[\)][ ]*[;].*";
        if(regex_search(line, match, ex)){
            GATES.insert(pair<string,gates*>(match[2],new XOR(wires.find(match[4])->second, wires.find(match[5])->second, wires.find(match[3])->second, stoi(match[1]))));
            return;
        }
        ...
        regex ex("^[ ]*not[ ]*[#]([[:w:]])[ ]+([[:w:]]+)[ ]*[\(][ ]*([[:w:]]+)[ ]*[,][ ]*([[:w:]]+)[ ]*[\)][ ]*[;].*");
        smatch match;
        if(regex_search(line, match, ex)){
            GATES.insert(pair<string,gates*>(match[2],new NOT(wires.find(match[4])->second, wires.find(match[3])->second, stoi(match[1]))));
            return;
        }
    }
}
