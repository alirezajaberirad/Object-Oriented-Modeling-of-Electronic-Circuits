    ...
    while(gatesEncountered!=GATES.size()){
        for(auto it=GATES.begin(); it!=GATES.end(); ++it){
            if(it->second->Level == printingLevel){
                ...
                file<<"); //its controllability is: "<<it->second->outputControllability;
                file<<", and its output observability is: "<<it->second->outputObservability<<endl;
            }
        }
        printingLevel++;
    }
